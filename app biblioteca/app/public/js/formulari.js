...
function enviarPersona(event){
    ...
    fetch('http://localhost:3001/persones/addPersona', {
        method: 'POST',
        headers: {
...
function enviarRecurs(event){
    event.preventDefault();
    const titol = document.getElementById("titol").value;
    const NumExemplars = Number(document.getElementById("exemplars").value);
    ...
    fetch('http://localhost:3001/recursos/addMaterial', {
        method: 'POST',
        headers: {
            'Content-Type':'application/json'
        },
